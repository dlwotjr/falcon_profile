...
TEST_SRCS    = src/main.c $(FALCON512_SRC) $(COMMON_SRC)
TEST_CFLAGS  = $(CFLAGS) -O2

PROF_SRCS    = src/main_profile.c $(FALCON512_SRC) $(FALCON1024_SRC) $(COMMON_SRC)
...
.PHONY: all run profile_o0 profile_o3 result graph venv clean distclean
...
define run_op
	./$(1) $(2)
	mkdir -p results
	gprof $(1) gmon.out > results/analysis_$(2)_$(3).txt
	$(RM) gmon.out
endef
...
graph: venv
	...
	for op in $(OPERATIONS); do \
	    venv/bin/gprof2dot -f prof results/analysis_$${op}_O0.txt -o output_$${op}_O0.dot; \
	    venv/bin/gprof2dot -f prof results/analysis_$${op}_O3.txt -o output_$${op}_O3.dot; \
	    dot -Tpng output_$${op}_O0.dot -o graphs/output_$${op}_O0.png; \
	    dot -Tpng output_$${op}_O3.dot -o graphs/output_$${op}_O3.png; \
	    echo "PNG: graphs/output_$${op}_O0.png  graphs/output_$${op}_O3.png"; \
	done
