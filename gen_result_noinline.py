#!/usr/bin/env python3
"""Read gprof flat profiles and write result_noinline.md comparing O0 vs O3 (without -fno-inline)."""
...
lines = []
lines.append("# Falcon Profiling Results: O0 vs O3 (without `-fno-inline`)")
lines.append("")
...
lines.append("")
lines.append("- **Without `-fno-inline`, O3 inlines 20–116 functions per operation** — far more than the 1–7 seen with `-fno-inline`. The profile loses detail but reflects true execution.")
...
lines.append("- **Speedups are unchanged** from the `-fno-inline` run — the `-fno-inline` flag only affects profiling visibility, not the optimized binary's inlining decisions.")

with open("result_noinline.md", "w") as f:
    f.write("\n".join(lines) + "\n")

print("result_noinline.md written.")
